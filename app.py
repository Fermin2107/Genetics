...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:Gurruchaga46@localhost/animalesdb'
...
def ver_raza(raza_id):
    ...
    if orden == 'desc':
        query = query.order_by(Animal.rp.desc())
    else:
        query = query.order_by(Animal.rp.asc())
    animales = query.all()
